...
class TestRunAnalysisSuccess:
    ...
    @patch.object(CERCAS, "_CERCAS__fetch_rates", side_effect=fake_fetch)
...
class TestRunAnalysisInsufficientData:
    ...
    @patch.object(CERCAS, "_CERCAS__fetch_rates", return_value=[(datetime(2024, 1, 2), 4.0)])
    def test_single_data_point(self, _):
        c = ready_cercas()
        c.run_analysis()
        assert c.histogram is None

    @patch.object(CERCAS, "_CERCAS__fetch_rates", return_value=[])
