...
Requirements covered (SRS v1.1 + Annex No. 1):
...
def make_cercas(**overrides) -> CERCAS:
    """Fully configured CERCAS instance; override any field via kwargs."""
    c = CERCAS()
...
class TestGetPeriodKeyMonthly:
    """Annex 2.3.5: one window = 30 consecutive days starting from start_date."""
    ...
    def test_january_31st_is_window_1_not_window_0(self):
        """
        January has 31 days. With 30-day windows, Jan 31 = day 30 from Jan 1
        → must be window 1, NOT grouped with January calendar month (window 0).
        """
        c = make_cercas(aggregation_type=AggregationType.MONTHLY)
        jan_31 = datetime(2024, 1, 31)   # day 30 from 2024-01-01
        assert c.get_period_key(jan_31) == 1, (
            "BUG (Annex 2.3.5): Jan 31 is day 30 from Jan 1 → window 1; "
            "calendar-month grouping incorrectly puts it in January (window 0)"
        )
...
class TestGetPeriodKeyQuarterly:
    """Annex 2.3.6: one window = 90 consecutive days starting from start_date."""
    ...
    def test_day_179_from_start_is_still_window_1(self):
        c = make_cercas(aggregation_type=AggregationType.QUARTERLY)
        assert c.get_period_key(START + timedelta(days=179)) == 1

    def test_day_180_from_start_opens_window_2(self):
        c = make_cercas(aggregation_type=AggregationType.QUARTERLY)
        assert c.get_period_key(START + timedelta(days=180)) == 2

    def test_march_31_is_window_1_not_window_0(self):
        """
        Calendar Q1 = Jan 1 – Mar 31 (91 days from Jan 1 = day 90).
        With 90-day windows, day 90 → window 1, NOT Q1 (window 0).
        """
        c = make_cercas(aggregation_type=AggregationType.QUARTERLY)
        mar_31 = datetime(2024, 3, 31)   # day 90 from 2024-01-01
        assert c.get_period_key(mar_31) == 1, (
            "BUG (Annex 2.3.6): Mar 31 is day 90 from Jan 1 → window 1; "
            "calendar-quarter grouping incorrectly puts it in Q1 (window 0)"
        )
...
class TestAggregateChangesMonthly:
    """aggregate_changes must group by 30-day windows, not calendar months."""

    def test_two_changes_within_first_30_days_form_one_bucket(self):
        c = make_cercas(aggregation_type=AggregationType.MONTHLY)
        changes = [
            (START + timedelta(days=1),  0.01),   # window 0
            (START + timedelta(days=29), 0.02),   # window 0
        ]
    ...
    def test_change_on_day_30_creates_second_bucket(self):
        ...
        changes = [
            (START + timedelta(days=1),  0.01),   # window 0
            (START + timedelta(days=29), 0.02),   # window 0
            (START + timedelta(days=30), 0.03),   # window 1
        ]
        result = c.aggregate_changes(changes)
        assert len(result) == 2
        assert abs(result[0] - 0.03) < EPS
        assert abs(result[1] - 0.03) < EPS

    def test_january_spans_two_30_day_windows(self):
        """
        Jan 2 = day 1 (window 0), Jan 30 = day 29 (window 0),
        Jan 31 = day 30 (window 1 — still January!).
        Calendar-month grouping would produce 1 bucket; correct result is 2.
        """
        c = make_cercas(aggregation_type=AggregationType.MONTHLY)
        changes = [
        ...
        result = c.aggregate_changes(changes)
        assert len(result) == 2, (
            "BUG (Annex 2.3.5): aggregate_changes groups by calendar month; "
            "Jan 31 (day 30) must start a new 30-day window, not stay in January"
        )

    def test_three_30_day_windows_produce_three_buckets(self):
        c = make_cercas(aggregation_type=AggregationType.MONTHLY)
        changes = [
            (START + timedelta(days=5),  0.10),   # window 0
            (START + timedelta(days=35), 0.20),   # window 1
            (START + timedelta(days=65), 0.30),   # window 2
        ]
        result = c.aggregate_changes(changes)
        assert len(result) == 3
        assert abs(result[0] - 0.10) < EPS
        assert abs(result[1] - 0.20) < EPS
        assert abs(result[2] - 0.30) < EPS

    def test_bucket_values_are_sums_of_changes_within_window(self):
        c = make_cercas(aggregation_type=AggregationType.MONTHLY)
        changes = [
            (START + timedelta(days=1), 0.05),    # window 0
            (START + timedelta(days=10), -0.02),  # window 0
            (START + timedelta(days=20), 0.03),   # window 0
            (START + timedelta(days=35), 0.10),   # window 1
        ]
        result = c.aggregate_changes(changes)
        assert len(result) == 2
        assert abs(result[0] - 0.06) < EPS   # 0.05 - 0.02 + 0.03
...
class TestAggregateChangesQuarterly:
    """aggregate_changes must group by 90-day windows, not calendar quarters."""

    def test_changes_within_first_90_days_form_one_bucket(self):
        c = make_cercas(aggregation_type=AggregationType.QUARTERLY)
        changes = [
            (START + timedelta(days=1),  0.10),   # window 0
            (START + timedelta(days=89), 0.05),   # window 0
        ]
        result = c.aggregate_changes(changes)
        assert len(result) == 1
    ...
    def test_change_on_day_90_creates_second_bucket(self):
        c = make_cercas(aggregation_type=AggregationType.QUARTERLY)
        changes = [
            (START + timedelta(days=89), 0.10),   # window 0
            (START + timedelta(days=90), 0.05),   # window 1
        ]
        result = c.aggregate_changes(changes)
        assert len(result) == 2
        assert abs(result[0] - 0.10) < EPS
        assert abs(result[1] - 0.05) < EPS

    def test_q1_spans_two_90_day_windows(self):
        """
        Calendar Q1 = Jan 1 – Mar 31 = 91 days.
        day 89 = Mar 30 → window 0; day 90 = Mar 31 → window 1.
        Calendar-quarter grouping would produce 1 bucket (all in Q1); correct: 2.
        """
        c = make_cercas(aggregation_type=AggregationType.QUARTERLY)
        changes = [
        ...
        result = c.aggregate_changes(changes)
        assert len(result) == 2, (
            "BUG (Annex 2.3.6): aggregate_changes groups by calendar quarter; "
            "Mar 31 (day 90) must start a new 90-day window, not stay in Q1"
        )

    def test_two_full_90_day_windows_produce_two_buckets(self):
        c = make_cercas(aggregation_type=AggregationType.QUARTERLY)
        changes = [
            (START + timedelta(days=10),  0.20),   # window 0
            (START + timedelta(days=100), 0.40),   # window 1
        ]
        result = c.aggregate_changes(changes)
        assert len(result) == 2
        assert abs(result[0] - 0.20) < EPS
        assert abs(result[1] - 0.40) < EPS
...
class TestBuildHistogram:
    ...
    def test_frequency_sum_equals_number_of_input_values(self):
        ...
        hist = c.build_histogram(values)
        assert sum(freq for _, _, freq in hist) == len(values)

    def test_first_bin_starts_at_minimum_value(self):
        c = make_cercas(number_of_intervals=4)
        values = [1.0, 2.0, 3.0, 4.0]
        hist = c.build_histogram(values)
        assert hist[0][0] == pytest.approx(1.0)

    def test_last_bin_ends_at_maximum_value(self):
        c = make_cercas(number_of_intervals=4)
        values = [1.0, 2.0, 3.0, 4.0]
        hist = c.build_histogram(values)
        assert hist[-1][1] == pytest.approx(4.0)

    def test_consecutive_bins_share_boundaries(self):
        c = make_cercas(number_of_intervals=4)
        values = [1.0, 2.0, 3.0, 4.0]
        hist = c.build_histogram(values)
    ...
    def test_all_equal_values_all_counted(self):
        c = make_cercas(number_of_intervals=3)
        hist = c.build_histogram([0.5, 0.5, 0.5])
        assert sum(f for _, _, f in hist) == 3

    def test_each_bin_is_tuple_of_three_elements(self):
        c = make_cercas(number_of_intervals=2)
        hist = c.build_histogram([0.0, 1.0])
        for entry in hist:
            assert len(entry) == 3

    def test_bin_counts_are_non_negative(self):
        c = make_cercas(number_of_intervals=5)
        hist = c.build_histogram([0.1, 0.5, 0.9])
        for _, _, freq in hist:
            assert freq >= 0
...
# ===========================================================================
# run_analysis — end date derived from aggregation type (Annex 2.3.5/2.3.6)
# ===========================================================================
...
class TestRunAnalysisEndDateDerived:
    ...
    def test_monthly_end_date_is_start_plus_29_days(self, _):
        """Annex 2.3.5: monthly period = 30 days → end = start + 29."""
        c = make_cercas(aggregation_type=AggregationType.MONTHLY)
        c.run_analysis()
    ...
    def test_quarterly_end_date_is_start_plus_89_days(self, _):
        """Annex 2.3.6: quarterly period = 90 days → end = start + 89."""
        c = make_cercas(aggregation_type=AggregationType.QUARTERLY)
        c.run_analysis()
        assert c.end_date == START + timedelta(days=89)

    @patch.object(CERCAS, "fetch_rates", side_effect=fake_fetch)
    def test_monthly_produces_histogram(self, _):
        c = make_cercas(aggregation_type=AggregationType.MONTHLY)
        c.run_analysis()
        assert c.histogram is not None

    @patch.object(CERCAS, "fetch_rates", side_effect=fake_fetch)
    def test_quarterly_produces_histogram(self, _):
        c = make_cercas(aggregation_type=AggregationType.QUARTERLY)
        c.run_analysis()
        assert c.histogram is not None
    ...
    @patch.object(CERCAS, "fetch_rates", side_effect=fake_fetch)
    def test_rerun_with_different_n_overwrites_histogram(self, _):
        c = make_cercas(number_of_intervals=3)
        c.run_analysis()
        c.number_of_intervals = 8
        c.run_analysis()
    ...
    def test_frequency_sum_equals_number_of_aggregated_windows(self, _):
        """
        The sum of all histogram bin frequencies equals the number of aggregation
        windows produced by aggregate_changes — not the number of daily data points.
        """
        c = make_cercas(aggregation_type=AggregationType.MONTHLY, number_of_intervals=5)
        c.run_analysis()
        total_counts = sum(f for _, _, f in c.histogram)
        # aggregate_changes with 91 mocked data points (90 daily changes) and
        # 30-day windows: windows 0,1,2,3 → 4 aggregated values
