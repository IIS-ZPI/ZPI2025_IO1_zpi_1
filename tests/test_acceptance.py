...
SRS sections covered:
    ...
    2.3.x  Data processing pipeline (cross rate, daily changes)
...
_EUR_RATES_QUARTERLY = [
    (datetime(2023, 1, 3), 4.68), (datetime(2023, 1, 4), 4.70),
    (datetime(2023, 1, 5), 4.71), (datetime(2023, 1, 9), 4.74),
    (datetime(2023, 2, 1), 4.73), (datetime(2023, 2, 2), 4.72),
    (datetime(2023, 2, 6), 4.75), (datetime(2023, 3, 1), 4.69),
    (datetime(2023, 3, 2), 4.65), (datetime(2023, 3, 6), 4.67),
]

_USD_RATES_QUARTERLY = [
    (datetime(2023, 1, 3), 4.31), (datetime(2023, 1, 4), 4.33),
    (datetime(2023, 1, 5), 4.34), (datetime(2023, 1, 9), 4.36),
    (datetime(2023, 2, 1), 4.36), (datetime(2023, 2, 2), 4.35),
    (datetime(2023, 2, 6), 4.38), (datetime(2023, 3, 1), 4.32),
    (datetime(2023, 3, 2), 4.28), (datetime(2023, 3, 6), 4.30),
]
...
def test_uc1_full_analysis_produces_histogram():
    ...
    c = _make_cercas()
    with patch.object(c, "fetch_rates", side_effect=[_EUR_RATES_MONTHLY, _USD_RATES_MONTHLY]):
        c.run_analysis()
    assert c.histogram is not None
    assert len(c.histogram) == 5


def test_uc1_histogram_frequencies_sum_to_number_of_daily_changes():
    """
    UC-1 / Issue-28 regression: total frequency across all bins must equal
    the number of daily changes.

    Manual calculation (MONTHLY, EUR/USD):
        4 data points (Jan 3, 4, 5, 9) → 3 daily changes → sum(freq) = 3.
    """
    c = _make_cercas()
    with patch.object(c, "fetch_rates", side_effect=[_EUR_RATES_MONTHLY, _USD_RATES_MONTHLY]):
        c.run_analysis()
...
def test_uc1_quarterly_aggregation_produces_histogram():
    ...
    Manual calculation (QUARTERLY, EUR/USD):
        10 data points → 9 daily changes → sum(freq) = 9.
    """
    c = _make_cercas(agg=AggregationType.QUARTERLY, intervals=3)
    with patch.object(c, "fetch_rates", side_effect=[_EUR_RATES_QUARTERLY, _USD_RATES_QUARTERLY]):
        c.run_analysis()
    assert c.histogram is not None
    total_freq = sum(freq for _, _, freq in c.histogram)
    assert total_freq == 9
...
def test_uc2_analysis_blocked_when_start_missing(capsys):
    """UC-2: system must reject run_analysis when start date is not set."""
    c = CERCAS()
    with patch("app.core.cercas.validate_currency"):
        c.set_pair("EUR", "USD")
    c.set_type(AggregationType.MONTHLY)
...
def test_uc3_user_can_change_pair_and_rerun():
    ...
    eur_pln_rates = [(d, v * 1.1) for d, v in _EUR_RATES_MONTHLY]
...
def test_uc3_user_can_switch_aggregation_type_and_rerun():
    """
    UC-3: switching from MONTHLY to QUARTERLY fetches more data and
    produces more daily changes.

    Manual calculation:
        MONTHLY  (start+30d): 4 data points → 3 daily changes → sum(freq) = 3
        QUARTERLY (start+90d): 10 data points → 9 daily changes → sum(freq) = 9
    """
    c = _make_cercas(agg=AggregationType.MONTHLY, intervals=5)
    with patch.object(c, "fetch_rates", side_effect=[_EUR_RATES_MONTHLY, _USD_RATES_MONTHLY]):
        c.run_analysis()
    assert sum(freq for _, _, freq in c.histogram) == 3

    c.set_type(AggregationType.QUARTERLY)
    with patch.object(c, "fetch_rates", side_effect=[_EUR_RATES_QUARTERLY, _USD_RATES_QUARTERLY]):
        c.run_analysis()
    assert sum(freq for _, _, freq in c.histogram) == 9